...
Ghosts       : 4 enemies, random movement, touch = -20
...
GHOST_COLORS = [
    (1.00, 0.18, 0.18),   # Blinky — red
    (1.00, 0.60, 0.80),   # Pinky  — pink
    (0.18, 0.90, 0.90),   # Inky   — cyan
    (1.00, 0.60, 0.18),   # Clyde  — orange
]

_TITLE_BASE = "Pacman  |  W/A/S/D  |  R restart"

# ─────────────────────────────────────────────
#  MAZE  — list of wall rectangles (x, y, w, h)
#  (x,y) = bottom-left corner in world coords
# ─────────────────────────────────────────────
_WALLS = [
    # outer border
    (-390, -290, 780,  14),   # bottom
    (-390,  276, 780,  14),   # top
    (-390, -290,  14, 580),   # left
    ( 376, -290,  14, 580),   # right
...
def _wall_rects():
    """Return walls as (x, y, w, h) tuples."""
    return _WALLS


def _collides_wall(nx, ny, r=PACMAN_RADIUS):
    """True if a circle at (nx,ny) with radius r overlaps any wall."""
...
_pac_x      = 0.0
_pac_y      = 0.0
_pac_angle  = 0.0      # degrees — direction mouth faces
_mouth_t    = 0.0      # frame counter for mouth animation
_moving     = False
...
def _safe_pos(r=20.0):
    """Random position not inside any wall and not too close to origin."""
    for _ in range(200):
...
def _spawn_ghost(color):
    ...
    return {
        ...
        "color": color,
        "hit_timer": 0,   # frames of invincibility after being hit
    }


def _reset():
    global _pac_x, _pac_y, _pac_angle, _mouth_t, _moving
    global _score, _game_over, _prev_score, _prev_game_over
...
def _set_title():
    global _prev_score, _prev_game_over
    if _prev_score == _score and _prev_game_over == _game_over:
        return
    state = "GAME OVER" if _game_over else "RUNNING"
    glutSetWindowTitle(
        f"{_TITLE_BASE}  |  Score: {_score}  |  {state}".encode()
    )
    _prev_score     = _score
...
def _draw_pacman():
    # Mouth opens and closes: wedge angle oscillates 5° → 35°
    if _moving:
...
def _draw_ghost(g):
    ...
    # Body: upper semicircle via GL_TRIANGLE_FAN
    glBegin(GL_TRIANGLE_FAN)
    glVertex2f(cx, cy)
    for i in range(61):
        a = pi * i / 60          # 0 → π  (top half)
        glVertex2f(cx + r * cos(a), cy + r * sin(a))
    glEnd()

    # Body: rectangular lower half
    glBegin(GL_TRIANGLE_FAN)
    glVertex2f(cx, cy - r)
    glVertex2f(cx - r, cy)
    ...
    # Wavy skirt at the bottom (3 bumps)
...
def _update_ghosts():
    ...
    for g in _ghosts:
        ...
        # Wall collision → pick new random direction
        ...
        if g["hit_timer"] == 0:
            if hypot(_pac_x - g["x"], _pac_y - g["y"]) < GHOST_HIT_R:
                _score = max(SCORE_FLOOR, _score - 20)
                g["hit_timer"] = 90   # 1.5 s invincibility
...
def update():
    ...
    if _moving:
        _pac_angle = (180 / pi) * (pi - pi * (dy + 1) / 2
                      if dx == 0 else
                      (0 if dx > 0 else 180) + (dy * -30))
    ...
    for i, p in enumerate(_collectibles):
        if hypot(_pac_x - p["x"], _pac_y - p["y"]) < COLLECT_RADIUS:
            _score = max(SCORE_FLOOR, _score + p["value"])
...
def draw():
    ...
    draw_text(WORLD_LEFT + 18, WORLD_TOP - 26,
              f"Score: {_score} / {TARGET_SCORE}  |  Ghost touch = -20")
    draw_text(WORLD_LEFT + 18, WORLD_TOP - 48,
              "W/A/S/D move  |  Green +10  |  Red -10  |  R restart  |  ESC quit",
              color=(0.7, 0.75, 0.85))
